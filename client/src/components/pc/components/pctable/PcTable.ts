...
export default class PcTable extends Vue {
	...
	private itemClass(row): string {
		if (this.unit === 'pcUnit') {
			return this.pc.pc_unit.indexOf(row) === this.selectedRow ? 'td active' : 'td not-active';
		}
		return this.pc.system_case_unit.indexOf(row) === this.selectedRow ? 'td active' : 'td not-active';
	}
	...
	private async insertSerialNumber(props): Promise<void> {
		...
		const data = await pcApi.editSerialNumber(
			item,
			this.pc._id,
		);
		if (data.oldSystemCase) {
			this.$emit('updatePc', data.oldSystemCase);
		}
		const find: Unit = this.pc.pc_unit.find(
			(unit) => unit.i === item.i,
		);
	...
	private isEditableCell(item): boolean {
		...
		if (item.serial_number === this.pc.serial_number) {
			return false;
		}
